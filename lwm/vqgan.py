...
class VQGAN:
    def __init__(self, vqgan_checkpoint, replicate=False):
        assert vqgan_checkpoint != '', "Checkpoint path cannot be empty"
        self.replicate = replicate
        self.config = VQGANConfig.get_default_config()
        
        # Check if file exists
        if not os.path.exists(vqgan_checkpoint):
            raise FileNotFoundError(f"Checkpoint file not found: {vqgan_checkpoint}")
        
        try:
            # Try loading as JAX/Flax checkpoint
            self.params = self._load_checkpoint(vqgan_checkpoint)
        except Exception as e:
            raise ValueError(
                f"Failed to load VQGAN checkpoint: {str(e)}\n"
                "Make sure you're using a JAX/Flax compatible checkpoint.\n"
                "If you have a PyTorch checkpoint, you need to convert it first."
            ) from e
    ...
    def _load_checkpoint(self, checkpoint_path: str):
        ...
        try:
            with open(checkpoint_path, 'rb') as f:
                # Custom unpickler with safety checks
                params = pickle.load(f)
                
                # Verify basic structure of loaded parameters
                if not isinstance(params, dict):
                    raise ValueError("Loaded checkpoint is not a dictionary")
                
                # Check for expected keys (modify based on your model's structure)
                expected_keys = {'params'}  # Add your expected top-level keys
                if not all(key in params for key in expected_keys):
                    raise ValueError(f"Checkpoint missing expected keys: {expected_keys}")
            ...
            if "persistent_load" in str(e):
                raise ValueError(
                    "Checkpoint appears to be a PyTorch state dict. "
                    "Please convert it to JAX/Flax format first."
                ) from e
            raise
            
    @staticmethod
    def convert_pytorch_checkpoint(pytorch_path: str, output_path: str):
        """
        ...
        # Convert parameters to JAX/Flax format
        flax_params = {}
        for name, param in pytorch_state.items():
            ...
            # Restructure parameter name to match Flax conventions
            flax_name = name.replace('.', '/')
            flax_params[flax_name] = param_jax
        
        # Save as pickle
        with open(output_path, 'wb') as f:
            pickle.dump({'params': flax_params}, f)
