...
class VisionTextProcessor(object):
    ...
    def get_default_config(updates=None):
        config = ConfigDict()
        config.fields_from_example = 'image_path,label'
        config.subfield_separator = ' '
        config.add_bos_token = True
        config.add_eos_token = True
        config.prepend_text = ''
    ...
    def __iter__(self):
        ...
        for tokens, loss_masks, loc, index in self.parallel_example_iterator():
            ...
            while len(token_buffer) > chunk_size + 1:
                ...
                if self.config.use_data_sharded_loader and not self.config.return_local_batch and self._node_info:
...
class JsonVisionDataset(object):
    ...
    def __init__(self, config, tokenizer, text_processor, node_info=None):
        self.config = self.get_default_config(config)
        assert self.config.path != ''
        self._tokenizer = tokenizer
        self._text_processor = text_processor
    ...
    def _iter_pad(self):
        chunk_size = self.config.batch_size * self.config.seq_length
        if self.config.use_data_sharded_loader and self._node_info:
            local_batch_size = self.config.batch_size // self._node_info['dp_node_size']
        else:
            local_batch_size = self.config.batch_size
    ...
    def _iter_no_pad(self):
        ...
        if self.config.use_data_sharded_loader:
            if self._node_info:
                local_batch_size = self.config.batch_size // self._node_info.get('dp_node_size', 1)
            else:
                local_batch_size = self.config.batch_size  # Set to the default batch size if _node_info is not provided
        chunk_size = local_batch_size * self.config.seq_length
            ...
            while len(token_buffer) > chunk_size + 1:
                ...
                if self.config.use_data_sharded_loader and not self.config.return_local_batch and self._node_info:
                    if self._node_info:
                        ...
                        sp_nodes_size = 1  # Default to 1 if _node_info is not available
                    if self._node_info:
                        sp_nodes_rank = jax.process_index() % sp_nodes_size
                        assert self.config.seq_length % sp_nodes_size == 0, (self.config.seq_length, sp_nodes_size)
                        seq_chunk_size = self.config.seq_length // sp_nodes_size
                        batch = {k: v[:, sp_nodes_rank * seq_chunk_size:(sp_nodes_rank + 1) * seq_chunk_size] for k, v in batch.items()}
                    assert self.config.seq_length % sp_nodes_size == 0, (self.config.seq_length, sp_nodes_size)
                    seq_chunk_size = self.config.seq_length // sp_nodes_size
